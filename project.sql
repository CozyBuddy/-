...
(6, NULL, TO_DATE('2024-03-25 09:00', 'YYYY-MM-DD HH24:MI'), TO_DATE('2024-03-25 11:05', 'YYYY-MM-DD HH24:MI')
, 'Korea', 'Japan', 'admin001', 'KE2468', 'FUK.FUK', 'SEOUL.ICN', 18, 3, 'HL7553');
INSERT INTO Scplane VALUES 
(7, '스테이크', TO_DATE('2024-04-15 11:10', 'YYYY-MM-DD HH24:MI'), TO_DATE('2024-04-15 12:25', 'YYYY-MM-DD HH24:MI')
...
is
  ...
  vnum2 number;
begin
    select nvl( max(contrat_num),0) +1 into vnum2 from yesno;
    if ct1<>12 then 
    ...
    exception 
       ...
       insert into yesno values ( vnum2,'동의',to_char(sysdate,'yyyy-mm-dd'),ui,1);
       insert into yesno values ( vnum2+1,'동의',to_char(sysdate,'yyyy-mm-dd'),ui,2);
       insert into yesno values ( vnum2+2,decode(instr(ct2,'3'),0,'거부','동의'),to_char(sysdate,'yyyy-mm-dd'),ui,3);
       insert into yesno values ( vnum2+3,decode(instr(ct2,'4'),0,'거부','동의'),to_char(sysdate,'yyyy-mm-dd'),ui,4);
       insert into yesno values ( vnum2+4,decode(instr(ct2,'5'),0,'거부','동의'),to_char(sysdate,'yyyy-mm-dd'),ui,5);
...
create or replace procedure mk_flightuser_02
 ...
 is
    ...
    e exception;
 begin
        select userid into vvui from flightuser where userid = ui ;
        select count(password) into vpw from flightuser where userid=ui and password =pw;
        if vpw=0 then
        raise e ;
        end if;
        ...
        exception
            when e then
            raise_application_error(-20015,'비밀번호가 일치하지 않습니다.');
            when no_data_found then 
...
begin
    DBMS_OUTPUT.PUT_LINE(' ');
    DBMS_OUTPUT.PUT_LINE(' '||'◆ '||dd||' '||dap||'에서 출발해 '||aap||'에 도착하는 비행기 일정 입니다 ◆');
    DBMS_OUTPUT.PUT_LINE(' ');
    DBMS_OUTPUT.PUT_LINE('------------------------------------------------------------------');
    if aap in ( 'SEOUL','JEJU','INCHEON') THEN 
     ...
     if vreserv%notfound then
        ...
        for rec in vreserv loop 
            DBMS_OUTPUT.PUT_LINE(' ');
            dbms_output.put_line('     출발시각 : '|| rec.출발시각 || ' 도착시각 : ' ||rec.도착시각 );
            dbms_output.put_line('      출발공항 : '|| rec.출발공항 ||' 도착공항 : '||rec.도착공항 || ' 소요시간 : '|| rec.소요시간 );
            dbms_output.put_line('   정상 운임 : ' ||rec.정상운임 ||'원'||'할인 운임 : ' || round(rec.정상운임*0.85,-2) ||'원'||' 특가 운임 : ' ||round(rec.정상운임*0.55,-2)||'원' );
            DBMS_OUTPUT.PUT_LINE(' ');
            DBMS_OUTPUT.PUT_LINE('----------------------------------------------------------------');
         end loop;
                dbms_output.put_line('             §국내선 무료수화물 갯수는 1개 입니다.§'); 
                DBMS_OUTPUT.PUT_LINE('   공항체크인시 초과무게 1KG 당 2000원의 추가요금이 발생합니다!');
       ...
       if vreserv2%notfound then
        ...
        for rec2 in vreserv2 loop 
            DBMS_OUTPUT.PUT_LINE(' ');
            dbms_output.put_line('     출발시각 : '|| rec2.출발시각 || ' 도착시각 : ' ||rec2.도착시각 );
            dbms_output.put_line('      출발공항 : '|| rec2.출발공항 ||' 도착공항 : '||rec2.도착공항 || ' 소요시간 : '|| rec2.소요시간);
            dbms_output.put_line(' 정상 운임 : ' ||rec2.정상운임 ||'원 '||'할인 운임 : ' || round(rec2.정상운임*0.85,-2) ||'원'||' 특가 운임 : ' ||round(rec2.정상운임*0.55,-2)||'원' );
            DBMS_OUTPUT.PUT_LINE(' ');
            DBMS_OUTPUT.PUT_LINE('------------------------------------------------------------------');
        end loop;
            dbms_output.put_line( '                    ★수화물 초과 요금 안내★');
            DBMS_OUTPUT.PUT_LINE(' ');
            ...
            ef2 := rec.excess_fee ;
             dbms_output.put_line( '           수화물 갯수 : ' || ef ||'개  ☞ ' || '추가금액 : '|| ef2 || '원');
...
-----------------------수화물 규정 추가 --------------------
-----------------------수화물 규정 추가 --------------------
EXEC SE_LRULE_03(22,'일반석', 5, 250000,'국제선 미주');
----------------------------------------------------------
----------------------추가 수화물 요금 계산 ------------
---------------------
...
-----------------지역별 서비스 글 작성
-----------------지역별 서비스 글 작성
EXEC SE_CSERVICE_02 ( '889','동북아시아','대만','02-2518-2200', '','한국어/영어 : 매일 24시간 일본어 : 매일 9:00-19:00(일본 현지시간 기준) 중국어 : 평일 08:30-12:00, 13:00-17:30(대만 현지시간 기준, 주말/공휴일 제외)','#admin002');
